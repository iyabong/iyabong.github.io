---
title: "[Oracle 튜닝 연습] SCOTT 계정으로 SQL 실행계획 분석하기"
date: 2025-06-23
categories: [Oracle, SQL 튜닝]
tags: [oracle, sql, tuning, execution plan, scott]
---

오늘은 Oracle Autonomous Database(ADB) 환경에서 SCOTT 계정을 만들어,
기본 SQL 튜닝 실습을 진행해보았다.
...
> 단, 힌트 (`FULL`, `INDEX`, `NO_PARALLEL` 등)는 동일하게 잘 적용됨
...
- `EMP.SAL`에 인덱스 추가 후 `WHERE SAL >= 3000` 조건 성능 비교
- `GROUP BY` → `SORT GROUP BY` vs `HASH GROUP BY` 비교
- 서브쿼리 vs JOIN 실행계획 비교

---

📌 **실행계획 해석 연습 + 옵티마이저 힌트 실습**을 ADB에서 해보며  
실무에서도 바로 활용할 수 있는 감각을 기를 수 있었다.
